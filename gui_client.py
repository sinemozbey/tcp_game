...
COLOR_TIMEOUT = "#7F8C8D"       # Timeout (Gri)
...
class TimelineCanvas(tk.Frame):
    ...
    def add_packet_arrow(self, direction, kind, seq, ack, rwnd, length):
        ...
        if sender_name == ROLE_A_NAME:
            arrow_color = "#3498DB" # Mavi
        else:
            arrow_color = "#E67E22" # Turuncu
        ...
        if kind == "ACK": 
            ...
            line_width = 3
            arrow_color = COLOR_DANGER
        elif kind == "RETX":
            text_color = "#D35400"
        elif kind == "TIMEOUT":
            text_color = COLOR_TIMEOUT
            line_width = 2
            arrow_color = COLOR_TIMEOUT
...
class SingleClientGUI(tk.Frame):
    ...
    def _build_ui(self):
        ...
        score_card.columnconfigure(1, weight=0) 
        ...
        right_panel = ttk.Frame(middle_pane) 
    ...
    def animate_send(self, pkt, kind_override=None): 
        kind = kind_override if kind_override else pkt.type
        self.root.after(0, lambda: self.timeline.add_packet_arrow(
            "outgoing", kind, pkt.seq, pkt.ack, pkt.rwnd, pkt.length
        ))
    
    def animate_recv(self, pkt, kind_override=None): 
        kind = kind_override if kind_override else pkt.type
        self.root.after(0, lambda: self.timeline.add_packet_arrow(
            "incoming", kind, pkt.seq, pkt.ack, pkt.rwnd, pkt.length
        ))
...
class GameLogicGUI(GameLogic):
    ...
    def _poll_input_queue(self):
        ...
        while True:
            ...
            # --- INACTIVITY TIMEOUT (45s) KONTROLÜ ---
            if time.time() - wait_start > RESPONSE_TIMEOUT_SECONDS:
                # 1. Puan düşür
                self.scoreboard.my_timeout() # -1 Puan
                self.ui.update_scores(self.scoreboard.my_score, self.scoreboard.opponent_score)
                self.ui.append_log(f"⚠️ Inactivity Timeout! (-1 Point)")
                
                # 2. Karşıya puan kaybını bildir (Senkronizasyon)
                sync_pkt = Packet.error(comment="SYNC_TIMEOUT_PENALTY")
                self._send_packet(sync_pkt)
                
                # 3. Sayacı sıfırla ki her saniye ceza kesmesin
                wait_start = time.time()
                
                # Döngüye devam et (Sıra hala bizde)
                continue

            try:
                # Bloklamadan (kısa timeout ile) kontrol et
    ...
    def _send_packet(self, pkt: Packet):
        # SYNC paketleri (False Alarm veya Timeout)
        is_sync_packet = (pkt.type == "ERROR" and 
                          ("SYNC_FALSE_ALARM" in (pkt.comment or "") or
                           "SYNC_TIMEOUT_PENALTY" in (pkt.comment or "")))

        # Görselleştirme türünü belirle (Timeout için özel renk)
        vis_kind = pkt.type
        if "SYNC_TIMEOUT_PENALTY" in (pkt.comment or ""):
            vis_kind = "TIMEOUT"

        self.ui.append_log(f"Sent: {pkt.type} s={pkt.seq} a={pkt.ack}")
        self.ui.animate_send(pkt, kind_override=vis_kind)
    ...
    def _receive_packet(self) -> Packet:
        ...
        # --- DÖNGÜ İÇİNDE PAKET ALMA (SENKRONİZASYON PAKETLERİ İÇİN) ---
        while True:
            raw = None
            
            # İç döngü: recv_json timeout kontrolü
            while True:
                self._check_and_update_buffer()
                
                elapsed = time.time() - start_wait
                
                # [DÜZELTME]: Alıcı tarafında 5 saniyelik ek tolerans (Grace Period).
                # Böylece gönderici tam 45. saniyede paket gönderdiğinde 
                # alıcı "timeout" deyip bağlantıyı kesmez, paketi bekler.
                remaining = (RESPONSE_TIMEOUT_SECONDS + 5) - elapsed 
                ...
                try:
                    ...
                    continue # Henüz gelmedi, tekrar dene
            ...
            if "SYNC_TIMEOUT_PENALTY" in incoming_comment:
                self.scoreboard.opponent_timeout() # -1 Opponent
                self.ui.update_scores(self.scoreboard.my_score, self.scoreboard.opponent_score)
    ...
    def _respond_to_incoming(self, pkt: Packet) -> bool:
        if "SYNC_" in (pkt.comment or "").upper():
            return False
